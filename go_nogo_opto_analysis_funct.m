function [output] = go_nogo_opto_analysis_funct(filename, num, phase, day, date, cutoff_setting); %filename is the "c:\path", '9-7-21' and 'number'
...
t = 1;
    ...
    for b = 1:(length (wait_start_index) - 1) 
        ...
        trial_cell{b, :} = data(wait_start_index(b, 1): wait_start_index(b + 1, 1), :); % makes a cell array containing the data from every trial (defined by 8 event to 8 event) in each cell
            ...
            trials_light_off(b, :) = sum((trial_cell{b,:}(:,2)) == 40);
            trials_light_on(b, :) = sum((trial_cell{b,:}(:,2)) == 44);
...
% Opto analysis
    ...
    trials_light_off_hit = trials_reward .* trials_light_off;
    trials_light_off_miss = trials_miss .* trials_light_off;
    trials_light_off_correct_rejection = trials_correct_rejection .* trials_light_off;
    trials_light_off_false_alarm = trials_false_alarm .* trials_light_off;

    Hits_Light_Off = sum(trials_light_off_hit(data_cutoff_early:data_cutoff_late, 1));
    Misses_Light_Off = sum(trials_light_off_miss(data_cutoff_early:data_cutoff_late, 1));
    CRs_Light_Off = sum(trials_light_off_correct_rejection(data_cutoff_early:data_cutoff_late, 1));
    FAs_Light_Off = sum(trials_light_off_false_alarm(data_cutoff_early:data_cutoff_late, 1));

    Total_Go_Sound_Light_Off = Hits_Light_Off + Misses_Light_Off;
    Total_NoGo_Sound_Light_Off = CRs_Light_Off + FAs_Light_Off;

    Hit_Rate_Light_Off = Hits_Light_Off / Total_Go_Sound_Light_Off;
    FA_Rate_Light_Off = FAs_Light_Off / Total_NoGo_Sound_Light_Off;

            if Hit_Rate_Light_Off == 1
                dprime_hr_light_off = 1-1/(2*Total_Go_Sound_Light_Off);
            elseif Hit_Rate == 0
                dprime_hr_light_off = 0 + 1/(2*Total_Go_Sound_Light_Off);
            else
                dprime_hr_light_off = Hit_Rate_Light_Off;
            end

            if FA_Rate_Light_Off == 0
                dprime_fa_light_off = 0 + 1/(2*Total_NoGo_Sound_Light_Off);
            elseif FA_Rate == 1
                dprime_fa_light_off = 1-1/(2*Total_NoGo_Sound_Light_Off);
            else
                dprime_fa_light_off = FA_Rate_Light_Off;
            end

            [d, c] = dprime_simple(dprime_hr_light_off, dprime_fa_light_off);

            output.D_Prime_light_off = d;
            output.Response_Bias_light_off = c;


    trials_light_on_hit = trials_reward .* trials_light_on;
    trials_light_on_miss = trials_miss .* trials_light_on;
    trials_light_on_correct_rejection = trials_correct_rejection .* trials_light_on;
    trials_light_on_false_alarm = trials_false_alarm .* trials_light_on;

    Hits_Light_On = sum(trials_light_on_hit(data_cutoff_early:data_cutoff_late, 1));
    Misses_Light_On = sum(trials_light_on_miss(data_cutoff_early:data_cutoff_late, 1));
    CRs_Light_On = sum(trials_light_on_correct_rejection(data_cutoff_early:data_cutoff_late, 1));
    FAs_Light_On = sum(trials_light_on_false_alarm(data_cutoff_early:data_cutoff_late, 1));

    Total_Go_Sound_Light_On = Hits_Light_On + Misses_Light_On;
    Total_NoGo_Sound_Light_On = CRs_Light_On + FAs_Light_On;

    Hit_Rate_Light_On = Hits_Light_On / Total_Go_Sound_Light_On;
    FA_Rate_Light_On = FAs_Light_On / Total_NoGo_Sound_Light_On;

            if Hit_Rate_Light_On == 1
                dprime_hr_Light_On = 1-1/(2*Total_Go_Sound_Light_On);
            elseif Hit_Rate == 0
                dprime_hr_Light_On = 0 + 1/(2*Total_Go_Sound_Light_On);
            else
                dprime_hr_Light_On = Hit_Rate_Light_On;
            end

            if FA_Rate_Light_On == 0
                dprime_fa_Light_On = 0 + 1/(2*Total_NoGo_Sound_Light_On);
            elseif FA_Rate == 1
                dprime_fa_Light_On = 1-1/(2*Total_NoGo_Sound_Light_On);
            else
                dprime_fa_Light_On = FA_Rate_Light_On;
            end

            [d, c] = dprime_simple(dprime_hr_Light_On, dprime_fa_Light_On);

            output.D_Prime_Light_On = d;
            output.Response_Bias_Light_On = c;

        output.Hit_Rate_Light_Off = Hit_Rate_Light_Off;
        output.Hit_Rate_Light_On = Hit_Rate_Light_On;

        output.FA_Rate_Light_Off = FA_Rate_Light_Off;
        output.FA_Rate_Light_On = FA_Rate_Light_On;

    output.Hits_Light_Off = Hits_Light_Off;
    output.Misses_Light_Off = Misses_Light_Off;
    output.CRs_Light_Off = CRs_Light_Off;
    output.FAs_Light_Off = FAs_Light_Off;
    output.Total_Go_Sound_Light_Off = Total_Go_Sound_Light_Off;
    output.Total_NoGo_Sound_Light_Off = Total_NoGo_Sound_Light_Off;

    output.Hits_Light_On = Hits_Light_On;
    output.Misses_Light_On = Misses_Light_On;
    output.CRs_Light_On = CRs_Light_On;
    output.FAs_Light_On = FAs_Light_On;
    output.Total_Go_Sound_Light_On = Total_Go_Sound_Light_On;
    output.Total_NoGo_Sound_Light_On = Total_NoGo_Sound_Light_On;

% Latencies

trials_light_off_index = find(trials_light_off(data_cutoff_early:data_cutoff_late, 1) == 1);
trials_light_on_index = find(trials_light_on(data_cutoff_early:data_cutoff_late, 1) == 1);

output.RW_latencies_light_off = RW_latencies(trials_light_off_index, 1);
output.RW_latencies_light_on = RW_latencies(trials_light_on_index, 1);

output.FA_latencies_light_off = FA_latencies(trials_light_off_index, 1);
output.FA_latencies_light_on = FA_latencies(trials_light_on_index, 1);


output.mean_RW_latency_light_off = mean(RW_latencies(trials_light_off_index, 1), "omitnan");
output.mean_RW_latency_light_on = mean(RW_latencies(trials_light_on_index, 1), "omitnan");

output.mean_FA_latency_light_off = mean(FA_latencies(trials_light_off_index, 1), "omitnan");
output.mean_FA_latency_light_on = mean(FA_latencies(trials_light_on_index, 1), "omitnan");

output.median_RW_latency_light_off = median(RW_latencies(trials_light_off_index, 1), "omitnan");
output.median_RW_latency_light_on = median(RW_latencies(trials_light_on_index, 1), "omitnan");

output.median_FA_latency_light_off = median(FA_latencies(trials_light_off_index, 1), "omitnan");
output.median_FA_latency_light_on = median(FA_latencies(trials_light_on_index, 1), "omitnan");

output.std_RW_latency_light_off = std(RW_latencies(trials_light_off_index, 1), "omitnan");
output.std_RW_latency_light_on = std(RW_latencies(trials_light_on_index, 1), "omitnan");

output.std_FA_latency_light_off = std(FA_latencies(trials_light_off_index, 1), "omitnan");
output.std_FA_latency_light_on = std(FA_latencies(trials_light_on_index, 1), "omitnan");


% Trial Log

    trial_log_uncut(:, 2) = (trials_light_off_hit(trials_analyzed_index, 1) .* 5) + (trials_light_off_miss(trials_analyzed_index, 1) .* 4) + (trials_light_off_correct_rejection(trials_analyzed_index, 1) .* 3) + (trials_light_off_false_alarm(trials_analyzed_index, 1) .* 2) + (trials_light_on_hit(trials_analyzed_index, 1) .* 15) + (trials_light_on_miss(trials_analyzed_index, 1) .* 14) + (trials_light_on_correct_rejection(trials_analyzed_index, 1) .* 13) + (trials_light_on_false_alarm(trials_analyzed_index, 1) .* 12) + (trials_restart(trials_analyzed_index, 1) .* 1);
    % 5 is hit, 4 is miss, 3 is CR, 2 is FA, 1 is restart
    % 15 is light on hit, 14 is light on miss, 13 is light on CR, 12 is light on FA

    trial_log_uncut(:, 1) = [1:size(trial_log_uncut, 1)]';
...
%     trial_log_norestarts = trial_log_cut(find(trial_log_cut(:, 2) ~= 1), :);
%     trial_log_uncut_norestarts = trial_log_uncut(find(trial_log_cut(:, 2) ~= 1), :);
% 
% output.trial_log_no_restarts = trial_log_norestarts;
% output.trial_log_uncut_norestarts = trial_log_uncut_norestarts;
