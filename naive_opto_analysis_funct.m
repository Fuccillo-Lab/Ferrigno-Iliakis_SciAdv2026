function [output] = naive_opto_analysis_funct(filename, num, phase, day, date, cutoff_setting); %filename is the "c:\path", '9-7-21' and 'number'
...
% Start up
    ...
    % Open and read .txt file 
        ...
        out = textscan(fileID, '%f %f %f %f %f %f %f', 'Delimiter', ',');
        file_struct = 7; % if changing number of commas in file, change this number too!
        ...
        trial_cell{b, :} = data(wait_start_index(b, 1): wait_start_index(b + 1, 1), :); % makes a cell array containing the data from every trial (defined by 8 event to 8 event) in each cell
            ...
            if ~isnan(reward_index(b, 1))
                ...
                % Opto 1 Analysis
                    ...
                    trials_light_on(b, :) = sum((trial_cell{b,:}(:,2)) == 100);
 
                   if trials_light_on(b, 1) > 0

                        try
                            light_on_end_index(b, 1) = find(trial_cell{b, :}(:, 2) == 101);
                            light_on_timestamps(b, 2) = trial_cell{b,:}(light_on_end_index(b, 1), 1);  
                            light_on_end_not_printed(b, 1) = 0;
                        catch
                            light_on_end_index(b, 1) = NaN;
                            light_on_timestamps(b, 2) = NaN;
                            light_on_end_not_printed(b, 1) = 1;
                        end

                       light_on_start_index(b, 1) = find(trial_cell{b, :}(:, 2) == 100);
                       light_on_timestamps(b, 1) = trial_cell{b,:}(light_on_start_index(b, 1), 1);
                   else
                       light_on_start_index(b, 1) = NaN;
                       light_on_end_index(b, 1) = NaN;
                       light_on_timestamps(b, 1) = NaN;
                       light_on_timestamps(b, 2) = NaN;  
                       light_on_end_not_printed(b, 1) = NaN;
                   end

                    trials_light_off(b, :) = sum((trial_cell{b,:}(:,2)) == 110);
 
                   if trials_light_off(b, 1) > 0

                        try
                            light_off_end_index(b, 1) = find(trial_cell{b, :}(:, 2) == 111);
                            light_off_timestamps(b, 2) = trial_cell{b,:}(light_off_end_index(b, 1), 1);  
                            light_off_end_not_printed(b, 1) = 0;
                        catch
                            light_off_end_index(b, 1) = NaN;
                            light_off_timestamps(b, 2) = NaN;
                            light_off_end_not_printed(b, 1) = 1;
                        end

                       light_off_start_index(b, 1) = find(trial_cell{b, :}(:, 2) == 110);
                       light_off_timestamps(b, 1) = trial_cell{b,:}(light_off_start_index(b, 1), 1);
                   else
                       light_off_start_index(b, 1) = NaN;
                       light_off_end_index(b, 1) = NaN;
                       light_off_timestamps(b, 1) = NaN;
                       light_off_timestamps(b, 2) = NaN;  
                       light_off_end_not_printed(b, 1) = NaN;
                   end
...
trials_light_on_index = find(trials_light_on == 1);

for i = 1:size(trials_light_on_index, 1)

trial_start = trial_start_timestamp(trials_light_on_index(i , 1), 1);
trial_zero = light_on_timestamps(trials_light_on_index(i , 1), 1); 
trial_licks = licking_timestamps{trials_light_on_index(i, 1), 1};

	if ~isnan(light_on_timestamps(trials_light_on_index(i , 1), 1))
		light_on_start = (light_on_timestamps(trials_light_on_index(i , 1), 1) - trial_zero) / 1000000; % in seconds
	else
		light_on_start = NaN;
	end

	if ~isnan(light_on_timestamps(trials_light_on_index(i , 1), 2))
		light_on_end = (light_on_timestamps(trials_light_on_index(i , 1), 2) - trial_zero) / 1000000; % in seconds
	else
		light_on_end = NaN;
	end

	
	if ~isempty(trial_licks)
		for a = 1:size(trial_licks, 1)
			trial_licks(a, 1) = trial_licks(a, 1) - trial_zero;
			trial_licks(a, 1) = trial_licks(a, 1) / 1000000; % in seconds
        end

        try
        lps_before_light_on(i, 1) = size(find(trial_licks >= (light_on_start - 1) & trial_licks < light_on_start), 1);
        catch
        lps_before_light_on(i, 1) = 0;
        end

        try
        lps_during_light_on(i, 1) = size(find(trial_licks >= (light_on_start) & trial_licks < light_on_end), 1);
        catch
        lps_during_light_on(i, 1) = 0;
        end

        try
        lps_after_light_on(i, 1) = size(find(trial_licks >= light_on_end), 1);
        catch
        lps_after_light_on(i, 1) = 0;    
        end
	else
		trial_licks = NaN;
        lps_before_light_on(i, 1) = 0; 
...
hold on
subplot(1, 2, 1)
plot(trial_licks, i, "|k", MarkerSize=1)
plot(light_on_start, i, "|r", MarkerSize=1)
plot(light_on_end, i, "|r", MarkerSize=1)

end

axis square
xlim([-1 2])
ylabel("Trials with Light")
xlabel("Time from Light ON (s)")
title("Light ON Licking", num)

trials_light_off_index = find(trials_light_off == 1);

for i = 1:size(trials_light_off_index, 1)

trial_start = trial_start_timestamp(trials_light_off_index(i , 1), 1);
trial_zero = light_off_timestamps(trials_light_off_index(i , 1), 1); 
trial_licks = licking_timestamps{trials_light_off_index(i, 1), 1};

	if ~isnan(light_off_timestamps(trials_light_off_index(i , 1), 1))
		light_off_start = (light_off_timestamps(trials_light_off_index(i , 1), 1) - trial_zero) / 1000000; % in secoffds
	else
		light_off_start = NaN;
	end

	if ~isnan(light_off_timestamps(trials_light_off_index(i , 1), 2))
		light_off_end = (light_off_timestamps(trials_light_off_index(i , 1), 2) - trial_zero) / 1000000; % in secoffds
	else
		light_off_end = NaN;
	end
	...
	if ~isempty(trial_licks)
		for a = 1:size(trial_licks, 1)
			trial_licks(a, 1) = trial_licks(a, 1) - trial_zero;
			trial_licks(a, 1) = trial_licks(a, 1) / 1000000; % in secoffds
		end

        try
        lps_before_light_off(i, 1) = size(find(trial_licks >= (light_off_start - 1) & trial_licks < light_off_start), 1);
        catch
        lps_before_light_off(i, 1) = 0;
        end

        try
        lps_during_light_off(i, 1) = size(find(trial_licks >= (light_off_start) & trial_licks < light_off_end), 1);
        catch
        lps_during_light_off(i, 1) = 0;
        end

        try
        lps_after_light_off(i, 1) = size(find(trial_licks >= light_off_end), 1);
        catch
        lps_after_light_off(i, 1) = 0;    
        end
...
subplot(1, 2, 2)
plot(trial_licks, i, "|k", MarkerSize=1)
plot(light_off_start, i, "|c", MarkerSize=1)
plot(light_off_end, i, "|c", MarkerSize=1)

end

axis square
xlim([-1 2])
ylabel("Trials without Light")
xlabel("Time from Fake Light (s)")
title("No Light Licking", num)
...
hold on
subplot(1, 2, 1)
ecdf(lps_before_light_on)
axis square
ylabel("Trials with Light")
xlabel("Licks per second")
title("Licking Distribution Before Light", num)

subplot(1, 2, 2)
ecdf(lps_before_light_off)
axis square
ylabel("Trials without Light")
xlabel("Licks per second")
title("Licking Distribution Before No Light", num)


output.lps_before_light_off = lps_before_light_off;
output.lps_during_light_off = lps_during_light_off;
output.lps_after_light_off = lps_after_light_off;

output.lps_before_light_on = lps_before_light_on;
output.lps_during_light_on = lps_during_light_on;
output.lps_after_light_on = lps_after_light_on;

output.mean_lps_pre_light_off = mean(output.lps_before_light_off, 'omitnan');
output.mean_lps_during_light_off = mean(output.lps_during_light_off, 'omitnan');
output.mean_lps_post_light_off = mean(output.lps_after_light_off, 'omitnan');

output.mean_lps_pre_light_on = mean(output.lps_before_light_on, 'omitnan');
output.mean_lps_during_light_on = mean(output.lps_during_light_on, 'omitnan');
output.mean_lps_post_light_on = mean(output.lps_after_light_on, 'omitnan');



output.mean_lps_difference_light_off_pre_minus_during = mean(lps_before_light_off - lps_during_light_off, 'omitnan');
output.std_lps_difference_light_off_pre_minus_during = std(lps_before_light_off - lps_during_light_off, 'omitnan');
output.n_lps_difference_light_off_pre_minus_during = size(lps_before_light_off - lps_during_light_off, 1);

output.mean_lps_difference_light_on_pre_minus_during = mean(lps_before_light_on - lps_during_light_on, 'omitnan');
output.std_lps_difference_light_on_pre_minus_during = std(lps_before_light_on - lps_during_light_on, 'omitnan');
output.n_lps_difference_light_on_pre_minus_during = size(lps_before_light_on - lps_during_light_on, 1);

output.mean_lps_difference_light_off_during_minus_after = mean(lps_during_light_off - lps_after_light_off, 'omitnan');
output.std_lps_difference_light_off_during_minus_after = std(lps_during_light_off - lps_after_light_off, 'omitnan');
output.n_lps_difference_light_off_during_minus_after = size(lps_during_light_off - lps_after_light_off, 1);

output.mean_lps_difference_light_on_during_minus_after = mean(lps_during_light_on - lps_after_light_on, 'omitnan');
output.std_lps_difference_light_on_during_minus_after = std(lps_during_light_on - lps_after_light_on, 'omitnan');
output.n_lps_difference_light_on_during_minus_after = size(lps_during_light_on - lps_after_light_on, 1);


%% Stats split by licking vs nonlicking pre stim

licking_before_light_off_index = find(lps_before_light_off > 0);
licking_before_light_on_index = find(lps_before_light_on > 0);

output.mean_lps_pre_light_off_licking = mean(output.lps_before_light_off(licking_before_light_off_index, 1), 'omitnan');
output.mean_lps_during_light_off_licking = mean(output.lps_during_light_off(licking_before_light_off_index, 1), 'omitnan');
output.mean_lps_post_light_off_licking = mean(output.lps_after_light_off(licking_before_light_off_index, 1), 'omitnan');

output.mean_lps_pre_light_on_licking = mean(output.lps_before_light_on(licking_before_light_on_index, 1), 'omitnan');
output.mean_lps_during_light_on_licking = mean(output.lps_during_light_on(licking_before_light_on_index, 1), 'omitnan');
output.mean_lps_post_light_on_licking = mean(output.lps_after_light_on(licking_before_light_on_index, 1), 'omitnan');

output.mean_lps_difference_light_off_pre_minus_during_licking = mean(lps_before_light_off(licking_before_light_off_index, 1) - lps_during_light_off(licking_before_light_off_index, 1), 'omitnan');
output.mean_lps_difference_light_on_pre_minus_during_licking = mean(lps_before_light_on(licking_before_light_on_index, 1) - lps_during_light_on(licking_before_light_on_index, 1), 'omitnan');

output.mean_lps_difference_light_off_during_minus_after_licking = mean(lps_during_light_off(licking_before_light_off_index, 1) - lps_after_light_off(licking_before_light_off_index, 1), 'omitnan');
output.mean_lps_difference_light_on_during_minus_after_licking = mean(lps_during_light_on(licking_before_light_on_index, 1) - lps_after_light_on(licking_before_light_on_index, 1), 'omitnan');

output.mean_lps_percentchange_light_off_pre_minus_during_licking = mean((lps_during_light_off(licking_before_light_off_index, 1) - lps_before_light_off(licking_before_light_off_index, 1)) ./ lps_before_light_off(licking_before_light_off_index, 1), 'omitnan');
output.mean_lps_percentchange_light_on_pre_minus_during_licking = mean((lps_during_light_on(licking_before_light_on_index, 1) - lps_before_light_on(licking_before_light_on_index, 1)) ./ lps_before_light_on(licking_before_light_on_index, 1), 'omitnan');


nolicking_before_light_off_index = find(lps_before_light_off == 0);
nolicking_before_light_on_index = find(lps_before_light_on == 0);

output.mean_lps_pre_light_off_nolicking = mean(output.lps_before_light_off(nolicking_before_light_off_index, 1), 'omitnan');
output.mean_lps_during_light_off_nolicking = mean(output.lps_during_light_off(nolicking_before_light_off_index, 1), 'omitnan');
output.mean_lps_post_light_off_nolicking = mean(output.lps_after_light_off(nolicking_before_light_off_index, 1), 'omitnan');

output.mean_lps_pre_light_on_nolicking = mean(output.lps_before_light_on(nolicking_before_light_on_index, 1), 'omitnan');
output.mean_lps_during_light_on_nolicking = mean(output.lps_during_light_on(nolicking_before_light_on_index, 1), 'omitnan');
output.mean_lps_post_light_on_nolicking = mean(output.lps_after_light_on(nolicking_before_light_on_index, 1), 'omitnan');

output.mean_lps_difference_light_off_pre_minus_during_nolicking = mean(lps_before_light_off(nolicking_before_light_off_index, 1) - lps_during_light_off(nolicking_before_light_off_index, 1), 'omitnan');
output.mean_lps_difference_light_on_pre_minus_during_nolicking = mean(lps_before_light_on(nolicking_before_light_on_index, 1) - lps_during_light_on(nolicking_before_light_on_index, 1), 'omitnan');
output.mean_lps_difference_light_off_during_minus_after_nolicking = mean(lps_during_light_off(nolicking_before_light_off_index, 1) - lps_after_light_off(nolicking_before_light_off_index, 1), 'omitnan');
output.mean_lps_difference_light_on_during_minus_after_nolicking = mean(lps_during_light_on(nolicking_before_light_on_index, 1) - lps_after_light_on(nolicking_before_light_on_index, 1), 'omitnan');



% Combine

output.trial_start_timestamps_light_off_licking = trial_start_timestamp(trials_light_off_index(licking_before_light_off_index , 1), 1);
output.stim_start_timestamps_light_off_licking = light_on_timestamps(trials_light_off_index(licking_before_light_off_index , 1), 1);
output.licking_timestamps_light_off_licking = licking_timestamps{trials_light_off_index(licking_before_light_off_index , 1), 1};

output.trial_start_timestamps_light_on_licking = trial_start_timestamp(trials_light_on_index(licking_before_light_on_index , 1), 1);
output.stim_start_timestamps_light_on_licking = light_on_timestamps(trials_light_on_index(licking_before_light_on_index , 1), 1);
output.licking_timestamps_light_on_licking = licking_timestamps{trials_light_on_index(licking_before_light_on_index , 1), 1};
